...
namespace polyglot {
    ...
    using PolyMove = uint16_t;
    using PolyWeight = uint16_t;
    using PolyLearn = uint32_t;
    ...
    constexpr std::array<Piece, 12> piece_to_poly_piece = {
        0, 2, 4, 6, 8, 10, 1, 3, 5, 7, 9, 11
    };

    PolyKey gen_poly_key(BoardState &state);
    std::vector<BookEntry> probe_book(PolyKey key);
    Move get_book_move(BookEntry& entry, BoardState& state);

    // Keys from http://hgm.nubati.net/book_format.html
    constexpr std::array<PolyKey, 781> polyglot_keys = {
